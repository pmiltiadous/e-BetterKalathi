...
async function fetchSupermarkets () {
  ...
  if (!cart.length || !town) {
    list.innerHTML = "<tr><td colspan='3' data-label='Status'>No cart or town data found.</td></tr>"
    return
  }
  ...
  try {
    ...
    console.error(err)
    list.innerHTML =
      "<tr><td colspan='3' data-label='Status'>Failed to fetch supermarket prices.</td></tr>"
  }
}
...
function renderSupermarkets() {
  ...
  if (!supermarketsData?.supermarkets?.length) {
    list.innerHTML = "<tr><td colspan='3' data-label='Status'>No supermarkets found.</td></tr>";
    return;
  }
  ...
  supermarkets.sort((a, b) => {
    ...
    return sortAscending ? a.total - b.total : b.total - a.total; 
  });

  list.innerHTML = supermarkets
    .map(
      (s, idx) => `
      <tr class="supermarket-main-row">
          <td data-label="Supermarket">
            <div class="supermarket-name-cell">
                ${
                  s.companyPhotoFile
                ...
                <div>
                    ...
                    ${
                      s.missing_count > 0
                        ? `<div style="color:red; font-size:0.85rem; font-weight:900; margin-top:0.25rem;">Missing ${s.missing_count} item(s)</div>`
                        : ''
                    }
                </div>
            </div>
          </td>
          <td data-label="Total Price">€${s.total.toFixed(2)}</td>
          <td data-label="Details">
              <button class="view-items-btn" data-idx="${idx}">View Items</button>
          </td>
      </tr>
      <tr id="breakdown-${idx}" class="breakdown-wrapper-row">
        <td colspan="3">
          <div class="item-breakdown-table">
              ...
              ${s.products
                .map(
                  p => `
                  <div class="item-breakdown-inner-row">
                      <span class="col name" data-col-label="Item">${p.name}</span>
                      <span class="col qty" data-col-label="Qty">${p.quantity}</span>
                      <span class="col total" data-col-label="Total">€${(p.price * p.quantity).toFixed(2)}</span>
                  </div>
              `
                )
                .join('')}
